"""Oracle connection pool with lifecycle management and Pythonic helpers.

Supports both ``oracledb`` (modern, pure Python / thin driver) and the legacy
``cx_Oracle`` driver.  The pool tracks borrowed connections, enforces a maximum
lifetime, pings idle connections before reuse, and wraps cursors so that column
names can be accessed case-insensitively while Oracle defaults are preserved.

Security notes
--------------
* Always use bind variables (``:n`` or ``:name``).  String formatting or
  concatenation into SQL is forbidden.
* Empty strings are converted to ``NULL`` on input because Oracle stores ``''``
  as ``NULL`` anyway; doing it explicitly avoids surprises when applications
  later read the value back as ``None``.
* CLOB values larger than 4000 characters are bound with the correct Oracle
  type so they are not silently truncated.
"""
...
# Oracle treats empty strings as NULL.  We make this explicit on the Python side
# so callers do not rely on a distinction that cannot be represented.
...
class Row:
    """Case-insensitive, lower-case and original-case row accessor.

    Oracle returns column names in UPPERCASE by default.  Applications that
    migrated from MySQL often expect lower-case keys.  ``Row`` supports all
    three access styles so existing code keeps working.
    """
...
class CursorWrapper:
    """Cursor wrapper that handles Oracle quirks transparently."""
    ...
    # ------------------------------------------------------------------
    # Bind-variable and empty-string helpers
    # ------------------------------------------------------------------
    @staticmethod
    def _convert_positional_args(args: Optional[Sequence[Any]]) -> Optional[list[Any]]:
        """Convert Python empty strings to None and prepare CLOBs."""
        if args is None:
            return None
        converted: list[Any] = []
        for value in args:
    ...
    def _set_clob_input_sizes(cursor: Any, params: Optional[Sequence[Any]]) -> None:
        """Tell Oracle which parameters are large text so CLOB binding is used."""
    ...
    # ------------------------------------------------------------------
    # Delegate everything else to the real cursor safely.
    # ------------------------------------------------------------------
    def __getattr__(self, name: str) -> Any:
...
class _PooledConnection:
    """A connection borrowed from the pool.  Tracks metadata for lifecycle checks."""
    ...
    def __del__(self) -> None:
        # Defensive: if the caller forgot to call close(), close the underlying
        # connection so we do not leak Oracle sessions.
        if self._in_use:
            try:
                self._conn.close()
            except Exception:
    ...
    def ping(self) -> bool:
        ...
        try:
            if hasattr(self._conn, "ping"):
                ...
                # Fallback: execute a trivial statement.
...
class ConnectionPool:
    """Oracle connection pool with lifecycle management.

    Parameters
    ----------
    dsn
        Oracle data source name or Easy Connect string.
    user, password
        Credentials.
    min_size, max_size
        Pool bounds.
    increment
        Number of connections created when the pool is exhausted.
    max_lifetime
        Maximum seconds a connection may be used before it is recycled.
    idle_timeout
        Seconds after which an idle connection is closed.
    ping_interval
        Seconds between mandatory liveness checks before a connection is reused.
    leak_detection_timeout
        If a connection is borrowed longer than this, a warning is logged.
    """
    ...
    # ------------------------------------------------------------------
    # Pool growth / connection creation
    # ------------------------------------------------------------------
    ...
    def acquire(self) -> _PooledConnection:
        ...
        with self._lock:
            ...
            # No available connection: grow the pool.
    ...
    def _maintenance_pass(self) -> None:
        now = time.monotonic()
        with self._lock:
            # Close idle connections that have been idle too long, but keep min_size.
            ...
            # Warn about possible leaked connections.
