...
10. **同端严格串行，禁止并发**：同一处理端（frontend 或 backend）下，任何时刻只允许存在 1 个 subagent 在执行。必须等上一个 subagent 完全结束后，才能 spawn 下一个。绝对不允许同端并行 spawn 多个 subagent。
11. **跨端严格串行**：若存在前端和后端两端，必须先跑完整个后端的处理流程，再开始前端。绝对不允许前后端并发执行。
...
跨端严格串行：若存在两个端，必须先跑完后端，再开始前端。不得两端并发。
